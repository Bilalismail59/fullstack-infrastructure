...
resource "google_compute_firewall" "allow_ssh" {
  ...
  source_ranges = [var.admin_cidr] # à limiter à ton IP d'admin prod
  target_tags   = ["web-server"]
  lifecycle { prevent_destroy = true }
}
...
# SSL Certificate (si tu veux du HTTPS)
...
resource "google_sql_database_instance" "main" {
  name             = "${var.project_name}-db"       # ICI on change le nom pour prod !
